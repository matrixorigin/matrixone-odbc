...
$dsnServer = if ($connect) { $Server } else { 'localhost' }
...
if (-not ('WindowsInstallerNative' -as [type])) {
    Add-Type @'
using System;
...
public static class WindowsInstallerNative {
    ...
    public static extern uint MsiInstallProductW(string packagePath, string commandLine);

    [DllImport("msi.dll", CharSet = CharSet.Unicode)]
    public static extern uint MsiEnableLogW(uint logMode, string logFile, uint logAttributes);

    [DllImport("msi.dll")]
    public static extern uint MsiSetInternalUI(uint uiLevel, ref IntPtr windowHandle);
}
'@
}

function Invoke-Msi([string]$PackagePath, [string]$Properties, [string]$LogName, [int[]]$AllowedExitCodes = @(0, 3010)) {
    $log = Join-Path $Artifacts $LogName
    Write-Host "Starting MSI scenario $LogName"
    $window = [IntPtr]::Zero
    [void][WindowsInstallerNative]::MsiSetInternalUI(2, [ref]$window)
    $logResult = [WindowsInstallerNative]::MsiEnableLogW(0x3fff, $log, 2)
    Assert-True ($logResult -eq 0) "MsiEnableLogW failed: $logResult"
    $commandLine = ($Properties + ' REBOOT=ReallySuppress').Trim()
    $exitCode = [WindowsInstallerNative]::MsiInstallProductW($PackagePath, $commandLine)
    Write-Host "Completed MSI scenario $LogName with exit code $exitCode"
    Assert-True ($exitCode -in $AllowedExitCodes) "Windows Installer exit $exitCode; see $log"
    return $exitCode
}
